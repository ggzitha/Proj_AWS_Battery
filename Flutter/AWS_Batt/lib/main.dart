...
class ParsedReading {
  final DateTime ts;
  final double voltx; // Voltage (pack)
  final double ampx;  // Current (A) (+ = charge, - = discharge)
  final double tempx; // Temperature (°C)
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  // ===== Persisted Settings (defaults shown; loaded from SQLite) =====
  double _vMax = 4.20; // per-cell
  double _vNom = 3.60; // per-cell
  double _vMin = 2.80; // per-cell
  int _seriesCells = 3; // default 3s
  int _parallelCells = 4; // default 4p
  int _cellMah = 3400; // mAh per cell
  double _rCellMilliOhm = 38; // mOhm
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      if (!(_vMin < _vNom && _vNom < _vMax)) {
        _vMin = 2.80; _vNom = 3.60; _vMax = 4.20;
      }
    });
  }
  ...
  Future<void> _saveSettings({
    ...
    await AppDb.instance.saveSettings(
      vmin: vmin, vnom: vnom, vmax: vmax,
      series: series, parallel: parallel,
      cellMah: cellMah, rCellMilliOhm: rMilliOhm,
    );
    setState(() {
      _vMax = vmax; _vNom = vnom; _vMin = vmin;
      _seriesCells = series; _parallelCells = parallel;
      _cellMah = cellMah; _rCellMilliOhm = rMilliOhm;
    });
  ...
  void _startScan() {
    ...
    if (now.isBefore(_nextAllowedScan)) {
      ...
      _pendingRetryTimer?.cancel();
      _pendingRetryTimer = Timer(Duration(milliseconds: max(150, waitMs)), () {
    ...
    ds.connSub = _ble.connectToDevice(
      ...
      switch (update.connectionState) {
        case DeviceConnectionState.connected:
          ds.connected = true;
          setState(() {});
          _ble.requestMtu(deviceId: ds.device.id, mtu: 247).catchError((_) {});
          _subscribe(ds);
          break;
  ...
  /// Tokenizer supports key=value and compact (v12.3a1.2t29.2)
  Map<String, String> _extractRawTokens(String raw) {
    final out = <String, String>{};
    String normNum(String s) { s = s.trim(); if (s.endsWith('.')) s = '${s}0'; return s; }

    final kv = RegExp(r'\b([a-zA-Z]+)\s*=\s*(-?\d+(?:\.\d*)?)\b', caseSensitive: false);
  ...
  Estimate? _computeEstimate(DeviceState ds) {
    ...
    final dtSec = max(1.0, tN.difference(t0).inMilliseconds / 1000.0);
    ...
    if (isChargingBySign) {
      if (slope > minSlope) {
        ...
        final secondsToFull = ((100.0 - sN) / max(pctPerSec, 1e-9)).clamp(0, 365 * 24 * 3600).toDouble();
        toFull = Duration(seconds: secondsToFull.round());
      }
    } else {
      if (slope < -minSlope) {
        ...
        final pctPerSec = (absI / (capAh * 3600.0)) * 100.0;
        final secondsToEmpty = (sN / max(pctPerSec, 1e-9)).clamp(0, 365 * 24 * 3600).toDouble();
        toEmpty = Duration(seconds: secondsToEmpty.round());
      }
    }
    ...
    final slopeUse = (isChargingBySign && (toFull != null))
        ? max(slope, (absI / (max((_cellMah / 1000.0) * _parallelCells, 1e-6) * 3600.0)) * 100.0)
        : (!isChargingBySign && (toEmpty != null))
        ? min(slope, -(absI / (max((_cellMah / 1000.0) * _parallelCells, 1e-6) * 3600.0)) * 100.0)
        : slope;
    ...
    if (totalSec > 0) {
      for (int s = 0; s <= totalSec; s += 10) {
        ...
        future.add(FlSpot(now.add(Duration(seconds: s)).millisecondsSinceEpoch.toDouble(), soc));
      }
    }
        ...
        builder: (_, controller) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Voltages row (3 columns) — no helperText inside fixed height
              Row(
                children: [
                  Expanded(
                    child: SizedBox(
              ...
              // Optional caption for defaults of voltage (kept subtle)
              Row(
                children: [
                  Expanded(child: Text("Default 2.80", style: captionStyle)),
                  Expanded(child: Center(child: Text("Default 3.60", style: captionStyle))),
                  Expanded(child: Align(alignment: Alignment.centerRight, child: Text("Default 4.20", style: captionStyle))),
                ],
              ),

              const SizedBox(height: 12),

              // Capacity (mAh) • Series • Parallel (3 columns, same height)
              Row(
                children: [
                  Expanded(
                    child: SizedBox(
                  ...
                  Expanded(
                    child: SizedBox(
                      ...
                      child: DropdownButtonFormField<int>(
                        value: parallelTmp,
                        onChanged: (v) { if (v != null) { parallelTmp = v; (ctx as Element).markNeedsBuild(); } },
                        items: [for (int i = 1; i <= 50; i++) i]
                            .map((e) => DropdownMenuItem(value: e, child: Text("${e}p")))
                            .toList(),
                        isDense: true,
                        decoration: const InputDecoration(
                          labelText: "Parallel",
              ...
              const SizedBox(height: 6),
              // Defaults exactly below those three panels
              Row(
                children: [
                  Expanded(child: Text("Default 3400", style: captionStyle)),
                  Expanded(child: Center(child: Text("Default 3s", style: captionStyle))),
                  Expanded(child: Align(alignment: Alignment.centerRight, child: Text("Default 4p", style: captionStyle))),
                ],
              ),

              const SizedBox(height: 12),

              // Internal resistance (mOhm) — no helperText inside fixed height
              Row(
                children: [
                  Expanded(
                    child: SizedBox(
              ...
              Row(
                children: [
                  Expanded(child: Text("Default 38.0", style: captionStyle)),
                ],
              ),
                  ...
                  FilledButton.icon(
                    onPressed: () async {
                      ...
                      if (vmax == null || vnom == null || vmin == null) {
                        _snack("Masukkan nilai tegangan yang valid."); return;
                      }
                      if (!(vmin < vnom && vnom < vmax)) {
                        _snack("Nilai V harus: Min < Nom < Max."); return;
                      }
                      if (rMilli == null || rMilli < 0 || rMilli > 1000) {
                        _snack("Nilai resistansi tidak valid (0–1000 mOhm)."); return;
                      }
                      if (cellMah == null || cellMah < 500 || cellMah > 100000) {
                        _snack("Capacity per cell (mAh) tidak valid (500–100000)."); return;
                      }

                      await _saveSettings(
                        vmax: vmax, vnom: vnom, vmin: vmin,
                        series: seriesTmp, parallel: parallelTmp,
                        cellMah: cellMah, rMilliOhm: rMilli,
                      );
                      if (ctx.mounted) Navigator.pop(ctx);
                    },
...
class _DeviceAccordion extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final metricPane = KeyedSubtree(
      key: ValueKey("metric-${state.device.id}-${state.last?.ts.millisecondsSinceEpoch ?? 0}"),
      child: _MetricPane(
        last: last,
...
class _MetricPane extends StatelessWidget {
  const _MetricPane({required this.last, this.rawVoltx, this.rawAmpx, this.rawTempx});
  ...
  String _showRawOrDouble(String? raw, double? num, {int maxDecimals = 2}) {
    final validRaw = (raw != null && RegExp(r'^-?\d+(?:\.\d+)?$').hasMatch(raw));
  ...
  Widget build(BuildContext context) {
    ...
    final tiles = [
      _MetricTile(label: "Voltage",     value: "${_showRawOrDouble(rawVoltx, r.voltx, maxDecimals: 2)} V"),
      _MetricTile(label: "Current",     value: "${_showRawOrDouble(rawAmpx,  r.ampx,  maxDecimals: 2)} A"),
      _MetricTile(label: "Temperature", value: "${_showRawOrDouble(rawTempx, r.tempx, maxDecimals: 2)} °C"),
      _MetricTile(label: "Watt",        value: "${(r.voltx * r.ampx).toStringAsFixed(2).replaceFirst(RegExp(r'\.?0+$'), '')} W"),
    ];
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(blurRadius: 6, offset: const Offset(0, 1), color: cs.shadow.withValues(alpha: 0.05))],
      ),
      child: Row(
        children: [
          Flexible(
        ...
        LineChartData(
          ...
          lineBarsData: [
            ...
            LineChartBarData(
              ...
              dotData: const FlDotData(show: false), color: Colors.yellowAccent,
            ),
            LineChartBarData(
